...
import './wxInquiry.scss'  
import api_wxInquiry from '../../api/api_wxInquiry'

declare var $:any;
@Component({
 name:'wxInquiry',
 template: require('./wxInquiry.html'),
})
export default class wxInquiry extends Vue {
    el: '#wxInquiry';
    skip = 0;
    /* 总询价编号 */
    ...
    load(searchingInquiryId, searchingState, skip, count) {
        ...
        if(this.pendingBack == true){
            if(this.searchingState == "0" || this.searchingState == "2"){
                ...
                api_wxInquiry.CspInquiry.getCspInquiryList(searchingInquiryId, "", searchingState, "", "", "", "", skip, count).then((res) => {
                ...
                api_wxInquiry.Inquiry.getInquiryList(searchingInquiryId, "", "", "",searchingState, "", "", skip, count).then((res) => {
                    if(res.data.length>0){
                        this.InquiryData=res.data;
                        this.InquiryData.forEach((item) => {
                            if(item.goodsName.length>7){
    ...
    load2(searchingInquiryId, searchingState,inquiryQueryTotal) {
        ...
        if(this.pendingBack == true){
            if(this.searchingState == "0" || this.searchingState == "2"){
                this.isPendingQuote = false;
                api_wxInquiry.CspInquiry.getCspInquiryList(searchingInquiryId, "", searchingState, "", "", "", "", 0,inquiryQueryTotal.totalCount+10).then((res) => {
                    var list = res.data;
                    list.forEach((item) => {
                        if(item.goodsName.length>7){
                            item.goodsName=item.goodsName.substr(0,7)+"..."
                ...
                this.isPendingQuote = true;
                api_wxInquiry.Inquiry.getInquiryList(searchingInquiryId, "", "", "",searchingState, "", "", 0,inquiryQueryTotal.totalCount+10).then((res) => {
                    var list = res.data;
                    list.forEach((item) => {
                        if(item.goodsName.length>7){
                            item.goodsName=item.goodsName.substr(0,7)+"..."
    ...
    LinckToInquiryDetail = function(item){
        if(item.status=='未处理' || item.status=='询价终结'){
            this.$router.go("/WxInquiry/WxInquiryReleaseDetail?id=" + item.id + '&status=' + item.status);
        }else{
            this.$router.go("/WxInquiry/WxInquiryDetail?id=" + item.id + '&status=' + item.status);
        }
    }
    ...
    /* 新增询价单 */
    inquiryAdd(){
        this.$router.push("/WxInquiry/WxInquiryAdd?id="+""+"&name="+"add");
    }
    /* 存储询价单查询条件 */
