...
import './onroad.scss'  
import api_onroad from '../../api/api_onroad'
...
export default class onroad extends Vue {
    el: '#onroad';
    /**  订单编号*/
    orderID:string='';
    /** 发货地址 */
    ...
    load(skip,count,orderStatus){
        api_onroad.Order.getOnWayOrderList(this.orderID,orderStatus,this.startAddress,this.endAddress,skip,count).then((res)=>{
        if(res.length>0){
                this.notnull=true;
                this.orderlist=res;
                res.forEach((item) => {
    ...
    load1(DataSkip,count,orderStatus){
        $.showLoading('加载中...');
         api_onroad.Order.getOnWayOrderList(this.orderID,orderStatus,this.startAddress,this.endAddress,DataSkip,count).then((res)=>{
            res.forEach((item) => {
                if(item.goodsName.length>7){
                    item.goodsName=item.goodsName.substr(0,7)+"..."
                }
    ...
    load2(orderQueryTotal,orderStatus){
        $.showLoading('加载中...');
        api_onroad.Order.getOnWayOrderList(this.orderID,orderStatus,this.startAddress,this.endAddress,0,orderQueryTotal.totalCount+this.count).then((res)=>{
            res.forEach((item) => {
                if(item.goodsName.length>7){
                    item.goodsName=item.goodsName.substr(0,7)+"..."
                }
